(() => {
  ...
  var messages = {
    'invalid-value': ({ paramName, validValueDescription, value }) => {
      ...
      return `The '${paramName}' parameter was given a value with an unexpected value. ${validValueDescription} Received a value of ${JSON.stringify(
        value
      )}.`;
    },
  ...
  function printCleanupDetails(deletedURLs) {
    ...
    if (deletionCount > 0) {
      logger.groupCollapsed(
        `During precaching cleanup, ${deletionCount} cached request${
          deletionCount === 1 ? ' was' : 's were'
        } deleted.`
      );
  ...
  function printInstallDetails(urlsToPrecache, urlsAlreadyPrecached) {
    ...
    if (precachedCount || alreadyPrecachedCount) {
      ...
      if (alreadyPrecachedCount > 0) {
        message += ` ${alreadyPrecachedCount} file${
  ...
  var _createCacheName2 = (cacheName) => {
    return [_cacheNameDetails2.prefix, cacheName, _cacheNameDetails2.suffix]
      .filter((value) => value && value.length > 0)
      .join('-');
  };
  ...
  var cacheNames2 = {
    ...
    getGoogleAnalyticsName: (userCacheName) => {
      return (
        userCacheName || _createCacheName2(_cacheNameDetails2.googleAnalytics)
      );
    },
    getPrecacheName: (userCacheName) => {
  ...
  var StrategyHandler = class {
    ...
    async fetch(input) {
      ...
      if (
        ...
        if (possiblePreloadResponse) {
          if (true) {
            logger2.log(
              `Using a preloaded navigation response for '${getFriendlyURL2(
                request.url
              )}'`
            );
          }
      ...
      try {
        ...
        if (true) {
          logger2.debug(
            `Network request for '${getFriendlyURL2(
              request.url
    ...
    async cachePut(key, response) {
      ...
      if (true) {
        ...
        if (vary) {
          logger2.debug(
            `The response for ${getFriendlyURL2(
      ...
      if (!response) {
        if (true) {
          logger2.error(
            `Cannot cache non-existent response for '${getFriendlyURL2(
              effectiveRequest.url
            )}'.`
          );
        }
      ...
      if (true) {
        logger2.debug(
          `Updating the '${cacheName}' cache with a new Response for ${getFriendlyURL2(
            effectiveRequest.url
          )}.`
        );
      }
    ...
    async _getResponse(handler, request, event) {
      ...
      try {
        ...
        if (!response) {
          ...
          logger2.log(
            `While responding to '${getFriendlyURL2(request.url)}', an ${
...
This is generally NOT safe. Learn more at https://bit.ly/wb-precache`;
  ...
  var Route = class {
    constructor(match, handler, method = defaultMethod) {
      if (true) {
        ...
        if (method) {
          finalAssertExports3.isOneOf(method, validMethods, {
            paramName: 'method',
          });
        }
      }
  ...
  var Router = class {
    ...
    handleRequest({ request, event }) {
      ...
      if (true) {
        logger3.groupCollapsed(
          `Router is responding to: ${getFriendlyURL3(url)}`
        );
      ...
      if (
        ...
        responsePromise = responsePromise.catch(async (err) => {
          if (catchHandler) {
            if (true) {
              logger3.groupCollapsed(
                `Error thrown when responding to:  ${getFriendlyURL3(
                  url
  ...
  var messages4 = {
    strategyStart: (strategyName, request) =>
      `Using ${strategyName} to respond to '${getFriendlyURL2(request.url)}'`,
  ...
  var StaleWhileRevalidate = class extends Strategy {
    ...
    async _handle(request, handler) {
      ...
      if (true) {
        logger2.groupCollapsed(
          messages4.strategyStart(this.constructor.name, request)
        );
